#!/usr/bin/env python3
"""
Voice Chatbot with LCD Face Animation
...
# Global variables for LCD animation
lcd_disp = None
...
def animate_speech():
    ...
    while not stop_animation.is_set():
        if all(s.exists() for s in SPEECH_SPRITES):
            ...
            # If sprites don't exist, show a simple animation with colors
...
def show_paused_screen():
    """Overlay a simple 'PAUSED' screen."""
    global lcd_disp
    if not lcd_disp:
        return
...
def main():
    ...
    if len(args) > 0:
        if args[0] == "--help":
            print("Voice Chatbot with LCD Face Animation")
